...
* Support for `mlogit` is deprecated. The reason is that the data structure for these models is one observation-choice per row. Every other model-fitting package supported by `my_marginaleffects` treats rows as individual observations. The observation-choice structure made it harder to track indices and match individual predictions to rows in the original data. This added a lot of complexity to `my_marginaleffects`, and the results were not always reliable or safe.
...
* Global option: `options("my_marginaleffects_print_omit" = "s.value")`
...
* `options(my_marginaleffects_safe = FALSE)` disables some safety checks and allows unadvisable (but potentially) useful features like *many* pairwise comparisons. Thanks to D.Locke for the feature request.
...
* `print.my_marginaleffects()` supports `style="tinytable"`, which returns a `tinytable` object. Call `print(avg_slopes(model))` to get a nice printed table in Quarto or Rmarkdown documents, via Typst, LaTeX or HTML. Default print format can be set using: `options(my_marginaleffects_print_style="tinytable")`
* `hypothesis` argument accepts a function which takes a `my_marginaleffects` data frame and returns a transformed data frame with `term` and `estimate` columns.
...
* `mvgam`: https://nicholasjclark.github.io/mvgam/ Not available on CRAN yet, but this package maintains its own `my_marginaleffects` support function.
...
* `column_names` argument in `print.my_marginaleffects()` to suppress the printed column names at the bottom of the printout.
...
* Results of the `predictions()` function could be inaccurate when (a) running version 0.15.0, (b) `type` is `NULL` or `invlink(link)`, (c) model is `glm()`, and (d) the `hypothesis` argument is non-numeric. Thanks to @strengejacke for report [#903](https://github.com/vincentarelbundock/my_marginaleffects/issues/903)
...
* New procedure `numderiv` argument use a different procedure to select the step size used in the finite difference numeric derivative used to compute standard errors: abs(x) * sqrt(.Machine$double.eps). The numerical results may not be exactly identical to previous versions of `my_marginaleffects`, but the step size should be adequate in a broader variety of cases. Note that users can use the `numderiv` argument for more control on numeric differentiation, as documented.
...
* `glmmTMB`: Standard errors are no longer supported because they may have been erroneous. Follow Issue #810 on Github for developments: https://github.com/vincentarelbundock/my_marginaleffects/issues/810
...
* `vcov()` and `coef()` methods for `my_marginaleffects` objects.
...
* Use the `numDeriv` package for numeric differentiation in the calculation of delta method standard error. A global option can now be passed to `numDeriv::jacobian`:
  - `options(my_marginaleffects_numDeriv = list(method = "simple", method.args = list(eps = 1e-6)))`
  - `options(my_marginaleffects_numDeriv = list(method = "Richardson", method.args = list(eps = 1e-6)))`
  - `options(my_marginaleffects_numDeriv = NULL)`
* Print:
  - Print fewer significant digits.
  - `print.my_marginaleffects` now prints all columns supplied to `newdata`
...
* The default value of `conf_level` in `summary()` and `tidy()` is now `NULL`, which inherits the `conf_level` value in the original `comparisons`/`my_marginaleffects`/`predictions` calls.
...
* For objects from `crch` package, a number of bugs have been fixed; standard errors should now be correct for `deltamethod()`, `my_marginaleffects()`, etc.
...
* New vignettes: 
  ...
  - Python Numpy models in `my_marginaleffects`. Thanks to timpipeseek.
...
* `by` is deprecated in `summary()` and `tidy()`. Use the same `by` argument in the main functions instead: `comparisons()`, `my_marginaleffects()`, `predictions()`
...
* `survival::clogit`
* `biglm`: The main quantities can be computed, but not the delta method standard errors. See https://github.com/vincentarelbundock/my_marginaleffects/issues/387
...
* Elasticity and semi-elasticity using the new `slope` argument in `my_marginaleffects()`: eyex, dyex, eydx
...
* New `by` arguments in `predictions()`, `comparisons()`, `my_marginaleffects()`
...
* `tidymodels` objects of class `tidy_model` are supported if the fit engine is supported by `my_marginaleffects`.
...
* Hypothesis Tests and Custom Contrasts using the Delta Method: https://my_marginaleffects.com/vignettes/hypothesis.html
* Multiple Imputation: https://my_marginaleffects.com/vignettes/multiple_imputation.html
* Causal Inference with the g-Formula: https://my_marginaleffects.com/vignettes/gcomputation.html
...
* `eps` in `my_marginaleffects()` is now "adaptive" by default: it equals 0.0001 multiplied the range of the predictor variable
...
* Bayesian Model Averaging for `brms` models using `type = "average"`. See vignette on the `my_marginaleffects` website.
* `eps` argument for step size of numerical derivative
* `my_marginaleffects` and `comparisons` now report confidence intervals by default.
...
New pages on the `my_marginaleffects` website: https://my_marginaleffects.com/
...
* Drop `numDeriv` dependency, but make it available via a global option:
  options("my_marginaleffects_numDeriv" = list(method = "Richardson", method.args = list(eps = 1e-5, d = 0.0001)))
* Bugfixes
* Documentation improvements
* CRAN tests
...
* `posterior_draws` function can be applied to a `predictions` or a
  `my_marginaleffects` object to extract draws from the posterior distribution.
...
* If the `data.table` package is installed, `my_marginaleffects` will automatically use it to speed things up.
* Contrast definition reported in a separate column of `my_marginaleffects` output.
