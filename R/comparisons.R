...
#'
#' * <https://my_marginaleffects.com/vignettes/comparisons.html>
#' * <https://my_marginaleffects.com/>
#'
...
#'     * Function that accepts a vector and returns a data frame with two columns of "lo" and "hi" values to compare. See examples below.
#'   - Numeric variables:
#'     * Numeric of length 1: Forward contrast for a gap of `x`, computed between the observed value and the observed value plus `x`. Users can set a global option to get a "center" or "backward" contrast instead: `options(my_marginaleffects_contrast_direction="center")`
...
#' + Warning: Avoid modifying your dataset between fitting the model and calling a `my_marginaleffects` function. This can sometimes lead to unexpected results.
...
#'   - Supported shortcuts strings: `r paste(names(my_marginaleffects:::comparison_function_dict), collapse = ", ")`
...
#'   - See examples below.
#'   - For more complex aggregations, you can use the `FUN` argument of the `hypotheses()` function. See that function's documentation and the Hypothesis Test vignettes on the `my_marginaleffects` website.
#' @param cross
...
#'
#' See `?print.my_marginaleffects` for printing options.
#'
...
comparisons <- function(model,
    ...
    newdata <- dedup_newdata(
        ...
        cross = cross,
        comparison = comparison)
    if (isFALSE(wts) && "my_marginaleffects_wts_internal" %in% colnames(newdata)) {
        wts <- "my_marginaleffects_wts_internal"
    }
    ...
    args <- list(model,
                 ...
                 hi = contrast_data[["hi"]],
                 lo = contrast_data[["lo"]],
                 wts = contrast_data[["original"]][["my_marginaleffects_wts_internal"]],
                 by = by,
                 marginalmeans = marginalmeans,
                 cross = cross,
                 hypothesis = hypothesis,
    ...
    if (!all(is.na(mfx[["my_marginaleffects_wts_internal"]]))) {
        my_marginaleffects_wts_internal <- mfx[["my_marginaleffects_wts_internal"]]
    } else {
        my_marginaleffects_wts_internal <- NULL
    }
    mfx[["my_marginaleffects_wts_internal"]] <- NULL

    out <- mfx

    data.table::setDF(out)

    out <- set_my_marginaleffects_attributes(
        out,
        get_my_marginaleffects_attributes(newdata, include_regex = "^newdata.*class|explicit|matrix|levels"))
    ...
    attr(out, "weights") <- my_marginaleffects_wts_internal
