get_contrast_data <- function(model,
    ...
    if (length(variable_classes) == 0) {
        ...
        variable_classes <- attr(newdata, "my_marginaleffects_variable_class")
    }
    if (length(attr(modeldata, "my_marginaleffects_variable_class")) == 0) {
    ...
    if (!isTRUE(cross)) {
        ...
        my_marginaleffects_ter <- unlist(ter, use.names = FALSE)
        my_marginaleffects_lab <- unlist(lab, use.names = FALSE)
        lo[, "term" := my_marginaleffects_ter]
        hi[, "term" := my_marginaleffects_ter]
        original[, "term" := my_marginaleffects_ter]
        lo[, "contrast" := my_marginaleffects_lab]
        hi[, "contrast" := my_marginaleffects_lab]
        original[, "contrast" := my_marginaleffects_lab]
