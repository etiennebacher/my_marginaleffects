...
get_ci_draws <- function(x, conf_level, draws, model = NULL) {
    ...
    } else if (identical("eti", getOption("my_marginaleffects_posterior_interval", default = "eti")) &&
        identical("median", getOption("my_marginaleffects_posterior_center", default = "median"))) {
        insight::check_if_installed("collapse", minimum_version = "1.9.0")
        # Issue #1017
        if (nrow(draws) > 0) {
    ...
    # faster known case
    if (identical("eti", getOption("my_marginaleffects_posterior_interval", default = "eti")) &&
        identical("mean", getOption("my_marginaleffects_posterior_center", default = "median"))) {
        insight::check_if_installed("collapse", minimum_version = "1.9.0")
    ...
    # option name change
    FUN_INTERVAL <- getOption("my_marginaleffects_posterior_interval")
    if (is.null(FUN_INTERVAL)) {
        FUN_INTERVAL <- getOption("my_marginaleffects_credible_interval", default = "eti")
    }
