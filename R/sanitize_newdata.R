...
build_newdata <- function(model, newdata, by, modeldata) {
    ...
    if (!inherits(newdata, "data.frame")) {
        msg <- "Unable to extract the data from model of class `%s`. This can happen in a variety of cases, such as when a `my_marginaleffects` package function is called from inside a user-defined function, or using an `*apply()`-style operation on a list. Please supply a data frame explicitly via the `newdata` argument."
...
add_wts_column <- function(wts, newdata, model) {
    ...
    if (isFALSE(wts)) {
        ...
        if (!is.character(wtsname) || length(wtsname) != 1 || !wtsname %in% colnames(newdata)) {
            ...
            newdata[["my_marginaleffects_wts_internal"]] <- newdata[[wtsname]]
            return(newdata)
        }
    }
    ...
    if (!isFALSE(wts)) {
        ...
        if (!flag1 && !flag2) {
            msg <- sprintf("The `wts` argument must be a numeric vector of length %s, or a string which matches a column name in `newdata`. If you did not supply a `newdata` explicitly, `my_marginaleffects` extracted it automatically from the model object, and the `wts` variable may not have been available. The easiest strategy is often to supply a data frame such as the original data to `newdata` explicitly, and to make sure that it includes an appropriate column of weights, identified by the `wts` argument.", nrow(newdata))
            stop(msg, call. = FALSE)
        }
    }
    ...
    if (!isFALSE(wts) && isTRUE(checkmate::check_string(wts))) {
        newdata[["my_marginaleffects_wts_internal"]] <- newdata[[wts]]
    } else {
        newdata[["my_marginaleffects_wts_internal"]] <- wts
    }
...
set_newdata_attributes <- function(model, modeldata, newdata, newdata_explicit) {
    ...
    vc <- attributes(modeldata)$my_marginaleffects_variable_class
    column_attributes <- list(
        "matrix_columns" = mc,
        "character_levels" = cl,
        "variable_class" = vc)
    newdata <- set_my_marginaleffects_attributes(newdata, column_attributes, prefix = "newdata_")

    # {modelbased} sometimes attaches useful attributes
    exclude <- c("class", "row.names", "names", "data", "reference")
    modelbased_attributes <- get_my_marginaleffects_attributes(newdata, exclude = exclude)
    newdata <- set_my_marginaleffects_attributes(newdata, modelbased_attributes, prefix = "newdata_")

    # original data
    attr(newdata, "newdata_modeldata") <- modeldata

    if (is.null(attr(newdata, "my_marginaleffects_variable_class"))) {
...
dedup_newdata <- function(model, newdata, by, wts, comparison = "difference", cross = FALSE, byfun = NULL) {
    ...
    if (!flag && (
        isFALSE(by) || # weights only make sense when we are marginalizing
            ...
            !is.null(byfun) ||
            !isFALSE(cross) ||
            isFALSE(getOption("my_marginaleffects_dedup", default = TRUE)))) {
        return(newdata)
    }

    vclass <- attr(newdata, "my_marginaleffects_variable_class")
    ...
    cols <- colnames(out)
    out <- out[, .("my_marginaleffects_wts_internal" = .N), by = cols]
    data.table::setDF(out)

    out[["rowid_dedup"]] <- seq_len(nrow(out))
    attr(out, "my_marginaleffects_variable_class") <- vclass
