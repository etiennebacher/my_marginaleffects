sanity_by <- function(by, newdata) {
    ...
    if (isTRUE(by == "group") && "group" %in% colnames(newdata)) {
        msg <- 'The "group" variable name is forbidden to avoid conflicts with the column names of the outputs produced by the `my_marginaleffects` package. Please rename your variable of change the value of the `by` argument.'
