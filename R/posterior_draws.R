#' Extract Posterior Draws or Bootstrap Resamples from `my_marginaleffects` Objects
#'
#' @param x An object produced by a `my_marginaleffects` package function, such as `predictions()`, `avg_slopes()`, `hypotheses()`, etc.
...
posterior_draws <- function(x, shape = "long") {
    ...
    if (is.null(attr(x, "posterior_draws"))) {
        warning('This object does not include a "posterior_draws" attribute. The `posterior_draws` function only supports bayesian models produced by the `my_marginaleffects` or `predictions` functions of the `my_marginaleffects` package.',
                call. = FALSE)
        return(x)
    }
...
average_draws <- function(data, index, draws, byfun = NULL) {
    ...
    w <- data[["my_marginaleffects_wts_internal"]]
    if (all(is.na(w))) {
        w <- NULL
    }
