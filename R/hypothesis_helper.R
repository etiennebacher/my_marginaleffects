...
#' @return `specify_hypothesis()` is a "function factory", which means that executing it will return a function suitable for use in the `hypothesis` argument of a `my_marginaleffects` function.
#' @export
#' @noRd
specify_hypothesis <- function(
    ...
    fun <- function(x) {
        ...
        x[, my_marginaleffects_internal_label := tmp]

        if (is.null(by)) {
            out <- x[, list(
                hypothesis = label(my_marginaleffects_internal_label),
