...
#'
#' # Use in `my_marginaleffects` to compute "Typical Marginal Effects". When used
...
datagrid_engine <- function(
    ...
    num <- Reduce(f = "*", num)
    if (isTRUE(num > 1e9)) {
        stop("You are trying to create a prediction grid with more than 1 billion rows, which is likely to exceed the memory and computational power available on your local machine. Presumably this is because you are considering many variables with many levels. All of the functions in the `my_marginaleffects` package include arguments to specify a restricted list of variables over which to create a prediction grid.", call. = FALSE)
    }
    ...
    # created by insight::get_data
    for (n in names(out)) {
        attr(out, "my_marginaleffects_variable_class") <- attr(dat, "my_marginaleffects_variable_class")
    }
...
datagridcf_internal <- function(
    ...
    variables_all <- tmp$all
    variables_manual <- names(at)
    variables_automatic <- c(tmp$automatic, "my_marginaleffects_wts_internal", "rowid_dedup")
...
prep_datagrid <- function(..., model = NULL, newdata = NULL, by = NULL) {
    ...
    attr_variable_classes <- attr(newdata, "my_marginaleffects_variable_class")
    ...
    if (!is.null(model) && is.null(newdata)) {
        ...
        variables_sub <- c(variables_sub, c("my_marginaleffects_wts_internal", "rowid_dedup"))
    ...
    attr(newdata, "my_marginaleffects_variable_class") <- attr_variable_classes
