get_hypothesis <- function(
    ...
    if (isTRUE(hypothesis %in% valid)) {
        safe <- isFALSE(getOption("my_marginaleffects_safe", default = TRUE))
        if (nrow(x) > 25 && !safe) {
            msg <- 'The "pairwise", "reference", and "sequential" options of the `hypotheses` argument are not supported for `my_marginaleffects` commands which generate more than 25 rows of results. Use the `newdata`, `by`, and/or `variables` arguments to compute a smaller set of results on which to conduct hypothesis tests.'
            insight::format_error(msg)
        }
    }
    ...
    insight::format_error("`hypotheses` is broken. Please report this bug: https://github.com/vincentarelbundock/my_marginaleffects/issues.")
}
...
eval_string_hypothesis <- function(x, hypothesis, lab) {
    ...
    if (isTRUE(grepl("\\bb\\d+\\b", hypothesis)) && !any(grepl("\\bb\\d+\\b", x[["term"]]))) {
        ...
        for (i in seq_len(nrow(x))) {
            tmp <- paste0("my_marginaleffects__", i)
            hypothesis <- gsub(paste0("b", i), tmp, hypothesis)
        }
        rowlabels <- paste0("my_marginaleffects__", seq_len(nrow(x)))
