...
get_predict.rq <- function(model,
    ...
    if (isTRUE(getOption("my_marginaleffects_linalg", default = "RcppEigen") == "RcppEigen")) {
        MM <- attr(newdata, "my_marginaleffects_model_matrix")
...
sanitize_model_specific.rqs <- function(model, ...) {
    stop("`my_marginaleffects` only supports `quantreg::rq` models with a single `tau` value.", call. = FALSE)
}
