my_marginaleffects_settings <- new.env()
...
settings_init <- function(settings = NULL) {
    ...
    default_settings <- list(
        my_marginaleffects_safefun_return1 = FALSE
    )
...
settings_get <- function(name) {
    if (name %in% names(my_marginaleffects_settings)) {
        get(name, envir = my_marginaleffects_settings)
    } else {
        NULL
    }
}
...
settings_set <- function(name, value) {
    assign(name, value = value, envir = my_marginaleffects_settings)
}

settings_rm <- function(name = NULL) {
    if (is.null(name)) {
        rm(list = names(my_marginaleffects_settings), envir = my_marginaleffects_settings)
    } else if ("name" %in% names(my_marginaleffects_settings)) {
        rm(list = name, envir = my_marginaleffects_settings)
    }
}
