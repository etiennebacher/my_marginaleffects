get_modeldata <- function(model, additional_variables = FALSE, modeldata = NULL, wts = FALSE, ...) {
    ...
    if (isTRUE(wts)) {
      ...
      if (is.numeric(tmp) && length(tmp) == nrow(out)) {
        out[["my_marginaleffects_wts_internal"]] <- tmp
      }
    }
...
set_variable_class <- function(modeldata, model = NULL) {
    ...
    if (is.null(model)) {
        attr(out, "my_marginaleffects_variable_class") <- cl
        return(out)
    }
...
get_variable_class <- function(newdata, variable = NULL, compare = NULL) {
    ...
    if ("my_marginaleffects_variable_class" %in% names(attributes(newdata))) {
        cl <- attributes(newdata)$my_marginaleffects_variable_class
    } else {
