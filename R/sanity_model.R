...
sanitize_model_specific.default <- function(model,
                                          vcov = NULL,
                                          calling_function = "my_marginaleffects",
                                          ...) {
    return(model)
}
...
sanity_model_supported_class <- function(model) {
    checkmate::assert_character(
        getOption("my_marginaleffects_model_classes", default = NULL),
        null.ok = TRUE)
    custom_classes <- getOption("my_marginaleffects_model_classes", default = NULL)
    ...
    if (isFALSE(flag)) {
        ...
        msg <- c(
            ...
            support,
            "",
            "New modeling packages can usually be supported by `my_marginaleffects` if they include a working `predict()` method. If you believe that this is the case, please file a feature request on Github: https://github.com/vincentarelbundock/my_marginaleffects/issues")
