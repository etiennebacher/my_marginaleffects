...
#' @export
sanitize_model_specific.mblogit <- function(model, calling_function = "my_marginaleffects", ...) {
    if (calling_function == "my_marginaleffects") {
        ...
        if (isTRUE(flag)) {
            stop("Cannot compute marginal effects for models of class `mblogit` when the data includes character variables. Please convert character variables to factors in the dataset before fitting the model, and call `my_marginaleffects` again.", call. = FALSE)
        }
    }
    return(model)
}
