get_contrast_data_factor <- function(model,
    ...
    if (is.factor(newdata[[variable$name]])) {
        ...
        msg <- "The `%s` variable is treated as a categorical (factor) variable, but the original data is of class %s. It is safer and faster to convert such variables to factor before fitting the model and calling a `my_marginaleffects` function." 
        msg <- sprintf(msg, variable$name, class(newdata[[variable$name]])[1])
        warn_once(msg, "my_marginaleffects_warning_factor_on_the_fly_conversion")
    ...
    if (is.factor(newdata[[variable$name]]) || isTRUE(convert_to_factor)) {
        lo[[variable$name]] <- factor(lo[["my_marginaleffects_contrast_lo"]], levels = levs)
        hi[[variable$name]] <- factor(hi[["my_marginaleffects_contrast_hi"]], levels = levs)
    } else {
        lo[[variable$name]] <- lo[["my_marginaleffects_contrast_lo"]]
        hi[[variable$name]] <- hi[["my_marginaleffects_contrast_hi"]]
    }

    contrast_label <- hi$my_marginaleffects_contrast_label
    contrast_null <- hi$my_marginaleffects_contrast_hi == hi$my_marginaleffects_contrast_lo

    tmp <- !grepl("^my_marginaleffects_contrast", colnames(lo))
...
contrast_categories_processing <- function(first_cross, levs_idx, levs, variable, newdata) {
    ...
    levs_idx <- stats::setNames(levs_idx, paste0("my_marginaleffects_contrast_", colnames(levs_idx)))
    if (!"my_marginaleffects_contrast_label" %in% colnames(levs_idx) || all(levs_idx$my_marginaleffects_contrast_label == "custom")) {
        levs_idx[, "my_marginaleffects_contrast_label" := paste0(my_marginaleffects_contrast_hi, ", ", my_marginaleffects_contrast_lo)]
    }
