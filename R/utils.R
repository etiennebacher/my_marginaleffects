...
get_my_marginaleffects_attributes <- function(x, exclude = NULL, include = NULL, include_regex = NULL) {
    out <- list()
...
set_my_marginaleffects_attributes <- function(x, attr_cache, prefix = "") {
...
sub_named_vector <- function(x, y) {
  ...
  if (length(stats::na.omit(idx)) > 0) {
    ...
    stop("set_coef() substitution error. Please report on Github with a reproducible example: https://github.com/vincentarelbundock/my_marginaleffects/issues", call. = FALSE)
  }
