...
get_predict.glmmTMB <- function(model,
    ...
    # see https://github.com/vincentarelbundock/my_marginaleffects/issues/1064
...
sanitize_model_specific.glmmTMB <- function(model, vcov = TRUE, re.form = NULL, ...) {
    ...
    if (!isTRUE(is.na(re.form))) {
        msg <- "For this model type, `my_marginaleffects` only takes into account the uncertainty in fixed-effect parameters. You can use the `re.form=NA` argument to acknowledge this explicitly and silence this warning."
