...
sanitize_variables <- function(variables,
    ...
    if (length(bad) > 0) {
        msg <- c(
            "These variable names are forbidden to avoid conflicts with the outputs of `my_marginaleffects`:",
    ...
    if (is.null(comparison)) {
        ...
        if (isTRUE(grepl("avg$", comparison)) && "my_marginaleffects_wts_internal" %in% colnames(newdata)) {
