...
#'
#' * <https://my_marginaleffects.com/vignettes/slopes.html>
...
#' + Warning: Please avoid modifying your dataset between fitting the model and calling a `my_marginaleffects` function. This can sometimes lead to unexpected results.
...
#' + Function:
#'   - Accepts an argument `x`: object produced by a `my_marginaleffects` function or a data frame with column `rowid` and `estimate`
...
#' + See the Examples section below and the vignette: https://my_marginaleffects.com/vignettes/hypothesis.html
...
#' + Extra arguments can be specified by passing a list to the `numDeriv` argument, with the name of the method first and named arguments following, ex: `numderiv=list("fdcenter", eps = 1e-5)`. When an unknown argument is used, `my_marginaleffects` prints the list of valid arguments for each method.
#' @param ... Additional arguments are passed to the `predict()` method
#' supplied by the modeling package.These arguments are particularly useful
#' for mixed-effects or bayesian models (see the online vignettes on the
#' `my_marginaleffects` website). Available arguments can vary from model to
...
slopes <- function(model,
    ...
    model <- sanitize_model(model = model, newdata = newdata, wts = wts, vcov = vcov, by = by, calling_function = "my_marginaleffects", ...)
    sanity_dots(model = model, calling_function = "my_marginaleffects", ...)
    ...
    class(out) <- c("slopes", "my_marginaleffects", class(out))
    return(out)
}
