...
plot_build <- function(
    ...
    dat$my_marginaleffects_term_index <- get_unique_index(dat, term_only = TRUE)
    multi_variables <- isTRUE(length(unique(dat$my_marginaleffects_term_index)) > 1)
    ...
    if (!is.null(v_facet_1) && !is.null(v_facet_2)) {
      fo <- stats::as.formula(paste(v_facet_2, "~", ifelse(multi_variables, "my_marginaleffects_term_index +", ""), v_facet_1))
      p <- p + ggplot2::facet_grid(fo,
                                   scales = "free",
                                   labeller = function(x){
                                     lapply(ggplot2::label_both(x), gsub, pattern = "my_marginaleffects_term_index: ", replacement="")
                                   })
    ## if pass only 1 facet then facet_wrap
    } else if (!is.null(v_facet_1) && is.null(v_facet_2)) {
        fo <- stats::as.formula(paste("~", ifelse(multi_variables, "my_marginaleffects_term_index +", ""), v_facet_1))
        p <- p + ggplot2::facet_wrap(fo, scales = "free")
    } else if (multi_variables) {
        fo <- stats::as.formula("~ my_marginaleffects_term_index")
        p <- p + ggplot2::facet_wrap(fo, scales = "free")
    }
