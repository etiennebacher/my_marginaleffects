bootstrap_fwb <- function(model, INF_FUN, ...) {
  ...
  # evaluate the {my_marginaleffects} call to get output without inferences()
  # use ... because arguments are not the same for different {my_marginaleffects} functions
  dots <- list(...)
  dots[["vcov"]] <- FALSE
  ...
  bootfun <- function(data, w) {
    ...
    # {my_marginaleffects} function needs to incorporate weights if
