get_contrasts <- function(model,
    ...
    out[, "my_marginaleffects_wts_internal" := NA_real_] # default (probably almost always overwritten)
    mult <- nrow(out) / nrow(original)
    regex <- "^term$|rowid_dedup|^group$|^contrast|^my_marginaleffects_wts_internal$"
    if (isTRUE(mult == 1)) {
    ...
    # cross-contrasts or weird cases
        ...
        if (isTRUE(nrow(out) == nrow(lo))) {
            tmp <- data.table(lo)[, .SD, .SDcols = patterns("^contrast|my_marginaleffects_wts_internal")]
            out <- cbind(out, tmp)
    ...
    if (!"my_marginaleffects_wts_internal" %in% colnames(out))  out[, "my_marginaleffects_wts_internal" := NA]
    ...
    if (isTRUE(marginalmeans)) {
        out <- out[, .(
            ...
            my_marginaleffects_wts_internal = mean(my_marginaleffects_wts_internal)),
        keyby = idx]
    }
    ...
    safefun <- function(hi, lo, y, n, term, cross, wts, tmp_idx, newdata) {
        ...
        if (length(con) == 1) {
            ...
            settings_set("my_marginaleffects_safefun_return1", TRUE)
        }
        return(con)
    }
    ...
    if (!is.null(draws)) {
        ...
        for (tn in unique(by_idx)) {
            for (i in seq_len(ncol(draws))) {
                ...
                draws[idx, i] <- safefun(
                    ...
                    term = out$term[idx],
                    cross = cross,
                    wts = out$my_marginaleffects_wts_internal[idx],
        ...
        # useless duplicates.
        if (!all(idx)) {
            if (settings_equal("my_marginaleffects_safefun_return1", TRUE)) {
                out[, "rowid" := NULL]
            }
            out <- out[idx, , drop = FALSE]
        }

        FUN_CENTER <- getOption("my_marginaleffects_posterior_center", default = stats::median)
    ...
    # reset settings
    settings_rm("my_marginaleffects_safefun_return1")
