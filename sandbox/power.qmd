---
title: Power analysis with `my_marginaleffects` and `DeclareDesign`
author: Vincent Arel-Bundock
date: 2023-01-14
---

This notebook shows how to conduct power analyses for quantities computed by the `my_marginaleffects` package `R`. We will use the simulation-based approach of the `DeclareDesign` suite.
...
Is the coefficient associated with Z different from 3? To answer this question, we can use the `my_marginaleffects::hypotheses()` function:
...
# estimator: mixed-effects model + {my_marginaleffects}
