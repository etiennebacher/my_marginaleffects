...
#' @inheritParams my_marginaleffects
...
#' of all the unit-level marginal effects computed by the `my_marginaleffects`
#' function.
#'
...
#' @details
#'
#' To compute standard errors around the average my_marginaleffects, we begin by applying the
...
tidy.comparisons <- function(x,
    ...
    my_marginaleffects_wts_internal <- attr(x, "weights")
    ...
    if (!is.null(my_marginaleffects_wts_internal)) {
        x_dt[, "my_marginaleffects_wts_internal" := my_marginaleffects_wts_internal]
    }
    ...
    # model
        ...
        # average marginal effects
        if ("my_marginaleffects_wts_internal" %in% colnames(x_dt)) {
        ...
        if (is.matrix(J) && is.matrix(V)) {
            ...
            idx_col_new <- paste0(idx_col_old, "_my_marginaleffects_index")
            setnames(idx_pad,
                     old = colnames(idx_pad),
                     new = paste0(colnames(idx_pad), "_my_marginaleffects_index"))
            ...
            tmp <- paste0(idx_by, "_my_marginaleffects_index")

            if (is.null(my_marginaleffects_wts_internal)) {
                J_mean <- J[, lapply(.SD, mean, na.rm = TRUE), by = tmp]
            } else {
                J[, "my_marginaleffects_wts_internal" := my_marginaleffects_wts_internal]
                J_mean <- J[,
                lapply(.SD,
                        stats::weighted.mean,
                        w = my_marginaleffects_wts_internal,
                        na.rm = TRUE),
                by = tmp]
            }
            if ("my_marginaleffects_wts_internal" %in% colnames(J_mean)) {
                tmp <- c("my_marginaleffects_wts_internal", tmp)
            }
            J_mean <- J_mean[, !..tmp]
...
tidy.predictions <- function(x,
    ...
    fun <- function(...) {
        ...
        if (!is.null(my_marginaleffects_wts_internal)) {
            out[, "my_marginaleffects_wts_internal" := my_marginaleffects_wts_internal]
            out <- out[,
                .(predicted = stats::weighted.mean(predicted, my_marginaleffects_wts_internal, na.rm = TRUE)),
                by = idx]
        } else {
